...
// Note about the cross reference in the fundoc: The autogenerated IDs lose the question mark from sub?,
// so sub and sub? get the same ID, distinguished only by the _2. The fundoc processor doesn't right
// now allow us to specify custom id, so we must just use the _sub_2 id here and try not to break it.

/* fundoc text
 The `sub` function is the subscription operator. It allows you to retrieve a member of an array (with an index)
 or an object (with a string). See also <<_sub_2>>.
 */
